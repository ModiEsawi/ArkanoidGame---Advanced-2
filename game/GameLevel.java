...
public class GameLevel implements Animation {
    ...
    public void initialize() {
        Block background = (Block) this.currentLevel.getBackground();
        ...
        int eachBlockHeight = 30, surfaceHeight = 800, surfaceWidth = 800, drawX = -1;
        ...
        this.remainedBalls.increase(this.currentLevel.numberOfBalls());
        ...
        List<Block> blocks = new ArrayList<>(this.currentLevel.blocks());
        this.remainedBlocks.increase(this.currentLevel.numberOfBlocksToRemove());
    ...
    public void run() {
        ...
        Paddle paddle = new Paddle(new Rectangle(paddleUpperleft, paddleWidth, paddleHeight), Color.YELLOW, sensor,
    ...
    public void doOneFrame(DrawSurface d) {
        ...
        if (this.remainedBalls.getValue() == 0) {
            ...
            Ball[] ballsArray = new Ball[this.currentLevel.numberOfBalls()];
            if (this.currentLevel.flag() != 4 && this.currentLevel.flag() != 2) {
                for (int i = 0; i < this.currentLevel.numberOfBalls(); i++) {
                    Ball ball = new Ball(this.currentLevel.ballsStartX() * (i + 1), this.currentLevel.ballsStartY(),
                            5, Color.white);
                    this.remainedBalls.increase(1);
                    ball.setVelocity(this.currentLevel.eachBallsVelocity());
                ...
                if (this.currentLevel.flag() == 4) { // fourth level information.
                    for (int i = 0; i < this.currentLevel.numberOfBalls(); i++) {
                        Ball ball = new Ball(this.currentLevel.ballsStartX() + (i * 50),
                                this.currentLevel.ballsStartY(), 5, Color.white);
                ...
                if (this.currentLevel.flag() == 2) { // second level information.
                    for (int i = 0; i < this.currentLevel.numberOfBalls(); i++) {
                        Ball ball = new Ball(this.currentLevel.ballsStartX() + (i * 90),
                                this.currentLevel.ballsStartY(), 5, Color.white);
                        // creating balls will increase the current ball counter.
                        this.remainedBalls.increase(1);
                        ball.setVelocity(this.currentLevel.eachBallsVelocity());
